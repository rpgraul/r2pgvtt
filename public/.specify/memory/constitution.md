...
### 3.6 Acessibilidade e Usabilidade de Formulários

```svelte
<!-- ✅ CORRETO — Label associado ao input via for/id -->
<label for="email">Email</label>
<Input id="email" bind:value={email} />

<!-- ✅ CORRETO — Checkbox com label clicável -->
<Checkbox id="visible" bind:checked={visible} />
<label for="visible">Visível para jogadores</label>

<!-- ✅ CORRETO — Select com label e id -->
<label for="category">Categoria</label>
<Select id="category" bind:value={category} items={items} />

<!-- ❌ ERRADO — Label sem associação -->
<label>Email</label>
<Input bind:value={email} />
```

**Regras:**
- Todo `<input>`, `<select>`, `<textarea>` **deve** ter um `<label>` associado
- Label **deve** ter atributo `for` correspondendo ao `id` do input
- Input **deve** ter atributo `id` correspondendo ao `for` do label
- Checkbox e Radio **devem** ter label clicável para alternar estado
- Select **deve** ter `id` e suportar keyboard (Escape para fechar)
- Todos os componentes de formulário em `src/components/ui/` **devem** aceitar prop `id`
