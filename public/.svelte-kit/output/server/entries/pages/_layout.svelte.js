...
function createDiceBoxManager(container, options = {}) {
  const {
    ...
    initPromise = (async () => {
      try {
        diceBoxInstance = new DiceBox({
          ...
          id: "dice-box-canvas",
          idExt: "",
          // Some versions of DiceBox append an extension. We can explicitly provide '' to keep it 'dice-box-canvas' if needed, or just let it be. But `id: 'dice-box-canvas'` handles it.
          assetPath,
...
function parseFormula(formula) {
  ...
  const result = {
    original: formula,
    baseFormula: "1d20",
    // O que vai pro 3D dice
    count: 1,
    sides: 20,
    explode: false,
    keep: null,
    // { type: 'highest' | 'lowest', count: Number }
    target: null,
    // { operator: string, value: Number }
    math: null
    // { operator: string, value: Number }
  };
...
function evaluateRolls(parsedData, rawRolls) {
...
function createDiceStore() {
  ...
  let diceBoxInstance2 = null;
  let diceBoxResolve = null;
  let diceBoxData = null;
  const defaultColor = "#0000ff";
  let currentDiceColor = typeof window !== "undefined" ? localStorage.getItem("rpgboard_dice_color") || defaultColor : defaultColor;
  function setDiceColor(color) {
    currentDiceColor = color;
    if (typeof window !== "undefined") {
      localStorage.setItem("rpgboard_dice_color", color);
    }
    if (diceBoxInstance2 && diceBoxInstance2.updateConfig) {
  ...
  function initDiceBox(container = null) {
    if (diceBoxInstance2) return Promise.resolve(diceBoxInstance2);
    if (diceInitializing) return diceInitializing;
    diceBoxInstance2 = createDiceBoxManager(container, {
    ...
    diceInitializing = diceBoxInstance2.init().then(() => {
  ...
  function rollDice(formula) {
    return new Promise(async (resolve, reject) => {
      ...
      isDiceVisible = true;
      if (!diceBoxInstance2 && !diceInitializing) {
      ...
      if (!diceBoxInstance2 || !diceBoxInstance2.isInitialized()) {
      ...
      try {
        await diceBoxInstance2.roll(parsedData.baseFormula);
      } catch (error) {
  ...
  function clearDice() {
    ...
    if (diceBoxInstance2) {
      diceBoxInstance2.clear();
    }
  }
  function clear3DDice() {
    if (diceBoxInstance2 && isDiceVisible && !hasActiveRolls()) {
      diceBoxInstance2.clear();
      isDiceVisible = false;
    }
  }
  function getDiceBox() {
    return diceBoxInstance2;
  }
  return {
    get activeDice() {
      return activeDice;
