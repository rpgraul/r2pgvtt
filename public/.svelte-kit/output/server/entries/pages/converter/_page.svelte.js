...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<main class="container px-4 py-6 max-w-4xl"><div class="flex items-center justify-between mb-6"><div><h1 class="text-2xl font-bold">Conversor de Cards</h1> <p class="text-sm text-muted-foreground mt-1">Converta texto formatado para JSON de cards</p></div> `);
    Badge($$renderer2, {
      children: ($$renderer3) => {
    ...
    if (notification) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<div${attr_class(`mb-4 p-3 rounded-lg ${stringify(notificationType === "success" ? "bg-green-500/20 text-green-400" : "bg-yellow-500/20 text-yellow-400")}`)}>${escape_html(notification)}</div>`);
    } else {
      $$renderer2.push("<!--[-1-->");
    }
...
...">`);
    ...
    if (jsonOutput) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<div><label class="text-sm font-medium mb-2 block">JSON de Saída</label> <textarea class="w-full h-64 p-4 rounded-lg border bg-muted font-mono text-sm resize-none" readonly="">`);
