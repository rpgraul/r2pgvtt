...
function Whiteboard($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(
      `<div class="h-full flex flex-col"><div class="flex items-center gap-2 p-2 border-b bg-card"><div class="flex gap-1"><!--[-->`,
    );
          ...
          Button($$renderer3, {
            ...
            children: ($$renderer4) => {
              if (tool.icon === 'cursor') {
                $$renderer4.push('<!--[0-->');
                $$renderer4.push(
                  `<svg class="w-4 h-4" viewBox="0 0 24 24" fill="currentColor"><path d="M4 4l16 8-8 2-2 8z"></path></svg>`,
                );
    ...
    $$renderer2.push(
      `<!--]--></div> <div class="w-px h-6 bg-border"></div> <div class="flex gap-1"><!--[-->`,
    );
    const each_array_1 = ensure_array_like(colors);
    for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
      let c = each_array_1[$$index_1];
      $$renderer2.push(
        `<button class="w-6 h-6 rounded border-2 transition-transform hover:scale-110"${attr_style(`background-color: ${stringify(c)}; border-color: ${stringify(color === c ? '#3b82f6' : 'transparent')}`)}${attr('aria-label', `Cor ${stringify(c)}`)}></button>`,
      );
    }
    $$renderer2.push(
      `<!--]--></div> <div class="w-px h-6 bg-border"></div> <div class="flex items-center gap-2"><input type="range" min="1" max="20"${attr('value', brushSize)} class="w-20"/> <span class="text-xs text-muted-foreground">${escape_html(brushSize)}px</span></div> <div class="flex-1"></div> `,
    );
    Button($$renderer2, {
      variant: 'ghost',
      size: 'sm',
    ...
    $$renderer2.push(
      `<!--]--></div> <div class="flex-1 bg-muted overflow-hidden"><canvas id="whiteboard-canvas"></canvas></div></div>`,
    );
  });
}
