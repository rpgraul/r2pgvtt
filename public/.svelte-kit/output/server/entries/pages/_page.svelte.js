...
function TrashDialog($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    async function handleEmptyTrash() {
      if (!confirm("Tem certeza que deseja LIMPAR A LIXEIRA? Todos os cards serão excluídos permanentemente.")) return;
      try {
          ...
          DialogContent($$renderer4, {
            ...
            children: ($$renderer5) => {
              ...
              DialogDescription($$renderer5, {
                children: ($$renderer6) => {
                  $$renderer6.push(`<!---->Cards excluídos são mantidos por 30 dias. Narrador pode restaurar ou excluir permanentemente.`);
                },
                $$slots: { default: true }
              });
              $$renderer5.push(`<!----> `);
              if (!gameState.isNarrator) {
                $$renderer5.push("<!--[0-->");
                $$renderer5.push(`<div class="text-center py-8 text-muted-foreground">Apenas o narrador pode acessar a lixeira.</div>`);
              } else if (isLoading) {
                $$renderer5.push("<!--[1-->");
                $$renderer5.push(`<div class="text-center py-8"><div class="animate-spin w-8 h-8 border-2 border-primary border-t-transparent rounded-full mx-auto"></div></div>`);
                ...
                for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
                  ...
                  $$renderer5.push(`<div class="flex items-center justify-between p-3 rounded-lg bg-muted/50"><div class="flex-1 min-w-0"><p class="font-medium truncate">${escape_html(item.titulo)}</p> <p class="text-xs text-muted-foreground">Excluído em ${escape_html(formatDate(item.deleted_at))} `);
                  if (isOlderThan30Days(item.deleted_at)) {
                    $$renderer5.push("<!--[0-->");
                    $$renderer5.push(`<span class="text-destructive font-medium">(expirando)</span>`);
                  } else {
                    $$renderer5.push("<!--[-1-->");
                  }
...
function CategoryFilters($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    function $$render_inner($$renderer3) {
      ...
      if (gameState.isNarrator) {
        $$renderer3.push("<!--[0-->");
        $$renderer3.push(`<button type="button" class="inline-flex items-center justify-center whitespace-nowrap rounded-sm px-3 py-1.5 text-sm font-medium ring-offset-background transition-all hover:bg-accent hover:text-accent-foreground">`);
...
function Card($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<div${attr_class(clsx(cn("group relative rounded-xl border bg-card text-card-foreground shadow-sm transition-all overflow-hidden", !item.isVisibleToPlayers && gameState.isNarrator && "border-amber-500/50 border-dashed")))}>`);
    if (item.imagemUrl) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<div class="relative aspect-[3/4] w-full overflow-hidden"><img${attr("src", item.imagemUrl)}${attr("alt", item.titulo)} class="h-full w-full object-cover"/> <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent"></div> `);
      if (item.category) {
        $$renderer2.push("<!--[0-->");
      ...
      $$renderer2.push(`<!--]--> <div class="absolute bottom-0 left-0 right-0 p-4"><h3 class="font-bold text-lg text-white line-clamp-2 drop-shadow-md">${escape_html(item.titulo)}</h3> `);
      if (item.tags && item.tags.length > 0) {
        ...
        for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
          ...
          $$renderer2.push(`<span class="text-xs bg-white/20 text-white/90 px-2 py-0.5 rounded-full backdrop-blur-sm">${escape_html(tag)}</span>`);
        }
        $$renderer2.push(`<!--]--></div>`);
      } else {
        $$renderer2.push("<!--[-1-->");
      ...
      $$renderer2.push("<!--[-1-->");
      $$renderer2.push(`<div class="p-4 space-y-3"><div class="flex items-start justify-between gap-2"><h3 class="font-bold text-lg line-clamp-2">${escape_html(item.titulo)}</h3> `);
      if (item.category) {
        $$renderer2.push("<!--[0-->");
        Badge($$renderer2, {
          variant: getCategoryVariant(item.category),
      ...
      if (item.conteudo) {
        $$renderer2.push("<!--[0-->");
        $$renderer2.push(`<div class="text-sm text-muted-foreground line-clamp-3">${escape_html(item.conteudo.replace(/<[^>]*>/g, ""))}</div>`);
      } else {
        $$renderer2.push("<!--[-1-->");
      }
      $$renderer2.push(`<!--]--> `);
      if (item.tags && item.tags.length > 0) {
        ...
        for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
          ...
          $$renderer2.push(`<span class="text-xs bg-muted text-muted-foreground px-2 py-0.5 rounded-full">${escape_html(tag)}</span>`);
        }
        $$renderer2.push(`<!--]--></div>`);
      } else {
        $$renderer2.push("<!--[-1-->");
    ...
    $$renderer2.push(`<!--]--> <div class="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1"><button class="bg-background/90 backdrop-blur-sm p-2 rounded-full hover:bg-primary hover:text-primary-foreground transition-colors" title="Editar">`);
    Square_pen($$renderer2, { class: "w-4 h-4" });
    $$renderer2.push(`<!----></button> <button class="bg-background/90 backdrop-blur-sm p-2 rounded-full hover:bg-destructive hover:text-destructive-foreground transition-colors" title="Excluir">`);
...
function GridContainer($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    function $$render_inner($$renderer3) {
      ...
      if (items().length === 0) {
        $$renderer3.push("<!--[0-->");
        $$renderer3.push(`<div class="flex flex-col items-center justify-center py-12 text-center"><div class="text-6xl mb-4">🎲</div> <h3 class="text-lg font-medium">Nenhum card encontrado</h3> <p class="text-sm text-muted-foreground mt-1">Tente ajustar seus filtros ou buscar por outro termo</p> `);
        Button($$renderer3, {
          class: "mt-4",
          onclick: openNewCard,
          children: ($$renderer4) => {
        ...
        $$renderer3.push("<!--[-1-->");
        $$renderer3.push(`<div${attr_class(clsx(cn("grid gap-4", gameState.viewMode === "grid" ? "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" : "grid-cols-1")))}><!--[-->`);
        const each_array = ensure_array_like(items());
        for (let index = 0, $$length = each_array.length; index < $$length; index++) {
          let item = each_array[index];
          $$renderer3.push(`<div draggable="true"${attr_class(clsx(cn("transition-all", dragOverIndex === index && "opacity-50 scale-95")))}>`);
          Card($$renderer3, { item });
          $$renderer3.push(`<!----></div>`);
        }
        $$renderer3.push(`<!--]--></div>`);
...
function TagFilters($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<div class="relative"><button type="button"${attr_class(clsx(cn("inline-flex items-center gap-2 px-3 py-2 text-sm rounded-md border transition-colors", selectedTags().length > 0 ? "bg-primary/20 border-primary text-primary" : "bg-muted border-input hover:bg-accent")))}><span>Tags</span> `);
    if (selectedTags().length > 0) {
      $$renderer2.push("<!--[0-->");
      $$renderer2.push(`<span class="bg-primary text-primary-foreground text-xs px-1.5 py-0.5 rounded-full">${escape_html(selectedTags().length)}</span>`);
    } else {
      $$renderer2.push("<!--[-1-->");
    }
    $$renderer2.push(`<!--]--></button> `);
...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    $$renderer2.push(`<div class="min-h-screen bg-background text-foreground"><main class="container px-4 py-6 space-y-6"><div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between"><div class="flex-1 max-w-md">`);
