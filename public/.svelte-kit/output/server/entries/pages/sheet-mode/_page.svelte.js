...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    function $$render_inner($$renderer3) {
      $$renderer3.push(
        `<main class="container px-4 py-6"><div class="flex items-center justify-between mb-6"><h1 class="text-2xl font-bold">Modo Ficha</h1> `,
      );
      Badge($$renderer3, {
        children: ($$renderer4) => {
      ...
      $$renderer3.push(
        `<!----></div> <div class="grid grid-cols-1 md:grid-cols-4 gap-6"><div class="md:col-span-1"><div class="rounded-lg border bg-card"><div class="p-3 border-b"><h2 class="font-semibold flex items-center gap-2">`,
      );
        ...
        children: ($$renderer4) => {
          ...
          if (characters().length === 0) {
            ...
            for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
              ...
              $$renderer4.push(
                `<button${attr_class(`w-full text-left p-3 rounded-lg border bg-card hover:bg-muted transition-colors ${stringify(selectedCard?.id === item.id ? 'border-primary' : '')}`)}>`,
              );
              if (item.imagemUrl) {
                $$renderer4.push('<!--[0-->');
                $$renderer4.push(
                  `<img${attr('src', item.imagemUrl)}${attr('alt', item.titulo)} class="w-full h-24 object-cover rounded-lg mb-2"/>`,
                );
              } else {
                $$renderer4.push('<!--[-1-->');
              }
              $$renderer4.push(
                `<!--]--> <h3 class="font-medium truncate">${escape_html(item.titulo)}</h3> <p class="text-xs text-muted-foreground truncate">${escape_html(item.category === 'pj' ? 'Personagem' : item.category)}</p></button>`,
              );
            }
            $$renderer4.push(`<!--]-->`);
          }
      ...
      $$renderer3.push(`<!----></div></div> <div class="md:col-span-3">`);
      {
        ...
        $$renderer3.push(
          `<!----> <p class="text-muted-foreground">Selecione um personagem para ver a ficha</p></div>`,
        );
      }
